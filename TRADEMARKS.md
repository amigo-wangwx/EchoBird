...
### 2. License posture (so the picture is clear)

EchoBird's public GitHub repository serves as the project's source
archive — not its distribution or promotion channel. Production
builds, support, and the product experience are offered through
[echobird.ai](https://echobird.ai). The public repository's source
is licensed as follows:

- **EchoBird v4.x and earlier**: AGPL-3.0-or-later. All v4.x binaries
  and their corresponding source remain under AGPL-3.0-or-later in
  perpetuity; this is not retroactively revocable.
- **EchoBird v5.0.0 and later**: [Business Source License 1.1](LICENSE).
  Source is published for transparency, audit, personal use, and
  honest forks. Production redistribution as a multi-LLM client
  product, hosted-service operation, or substantially overlapping
  derivative product requires a commercial license — see the
  Additional Use Grant in [LICENSE](LICENSE) for the precise scope.
  Each v5.x version auto-converts to GPL-2.0-or-later four years
  after its publication date.

The Rust core (`echobird_core`) ships as a vendored dependency under
the same license as the corresponding public release. Commercial
licensing inquiries for v5.x cover `echobird_core` as well.

The "trade dress" and "brand" protections discussed in this document
...
4. Keep LICENSE, NOTICE, and this TRADEMARKS.md intact. For forks based
   on v5.0.0 or later, the Business Source License's Additional Use Grant
   applies to your fork — see LICENSE for production-use restrictions
...
4. Keep LICENSE, NOTICE, and TRADEMARKS.md intact, and add one line in
   NOTICE crediting EchoBird as the upstream codebase. For forks based
   on v5.0.0 or later, the BSL Additional Use Grant applies to your fork.
...
open an issue at <https://github.com/edison7009/EchoBird/issues>.
Suggested labels: `trademark`, `trade-dress`.

Commercial licensing inquiries for v5.x (production redistribution,
hosted-service operation, or substantially overlapping derivative
product): email `hi@echobird.ai`.

AGPL §6 source requests for v4.x `echobird_core` (legacy): open an
issue with the `source-request` label.
...
EchoBird 的公开 GitHub 仓库**只是项目的源码归档** —— 不是分发渠道,
也不是宣传渠道。生产构建、技术支持、完整产品体验由
[echobird.ai](https://echobird.ai) 提供。公开仓库源码协议如下:

- **EchoBird v4.x 及以前版本**:AGPL-3.0-or-later。所有 v4.x 二进制及
  对应源码**永久**保留在 AGPL-3.0-or-later 协议下;不可溯及撤销。
- **EchoBird v5.0.0 及以后版本**:[Business Source License 1.1](LICENSE)。
  源码公开发布,供透明审计、个人使用、诚实 fork。**将源码作为多 LLM
  客户端产品再分发、运营 hosted service、或制作用户可见功能高度
  重叠的衍生品**,需要商业许可 —— 具体范围见 [LICENSE](LICENSE) 中
  Additional Use Grant 段落。每个 v5.x 版本自其发布日起 4 年后**自动
  转为 GPL-2.0-or-later**。

Rust 核心(`echobird_core`)作为 vendored 依赖随对应公开版本同协议授权。
v5.x 商业许可询价同时覆盖 `echobird_core`。
...
4. 保留 LICENSE、NOTICE 与本 TRADEMARKS.md 原貌。对基于 v5.0.0 及以后版本
   的 fork,Business Source License 的 Additional Use Grant 同样适用于你
   的 fork —— 你须遵守 LICENSE 中规定的生产使用限制。
...
3. 更新 `src/` 中的用户可见字符串。
4. 保留 LICENSE、NOTICE 与 TRADEMARKS.md 原貌,并在 NOTICE 中明确
   添加一行致谢 EchoBird 作为代码上游来源。对基于 v5.0.0 及以后版本的
   fork,BSL 的 Additional Use Grant 同样适用于你的 fork。
...
商标问题、授权申请、侵权举报:在
<https://github.com/edison7009/EchoBird/issues> 提交 issue。
建议标签:`trademark` / `trade-dress`。

**v5.x 商业许可询价**(生产再分发、hosted service 运营、用户可见
功能高度重叠的衍生品):邮件 `hi@echobird.ai`。

**v4.x 历史版本** `echobird_core` 的 AGPL §6 源码请求:提交带
`source-request` 标签的 issue。
