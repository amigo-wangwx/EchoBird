...
export const mdComponents: Components = {
  code: ({ node: _node, className, children, ...props }) => {
    ...
    return isInline ? (
      <code
        className="bg-cyber-text/10 text-cyber-text px-1.5 py-0.5 rounded text-[0.85em] font-mono"
        {...props}
      >
        {children}
      </code>
      ...
      <code
        className={`block bg-black/40 border border-cyber-border/30 rounded-lg p-3 pr-10 my-2 text-[0.85em] font-mono text-cyber-text overflow-x-auto whitespace-pre ${className || ''}`}
        {...props}
      >
        {children}
      </code>
        ...
        <button
          onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
            ...
            btn.insertAdjacentHTML(
              'beforeend',
              '<span class="copy-ok" style="color:#7aa2f7">✓</span>'
            );
            setTimeout(() => {
          ...
          className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center text-cyber-text-muted/40 hover:text-cyber-text transition-colors"
        >
          <svg
